...
def output_to_dannce(calib_path, base_path, output_file, frame_rate, length_ms):
    ...
    frame_count = frame_rate*length_ms
...
#only need to change things for this 3 lines
date_folder = '2024_05_03_mice_test'
calib_many = ['calib_before','calib_after']
project_folder = 'G:/Videos/6cam/jn187/'
frame_rate = 30
length_ms = 600
# 
base_path = os.path.join(project_folder, date_folder,'calibs_for_dannce')
for calib in calib_many:
    calib_path = os.path.join(project_folder, date_folder, calib)
    print(calib_path)
    output_file = f'test_240503_{calib}_label3d_dannce.mat'
    output_to_dannce(calib_path, base_path, output_file, frame_rate, length_ms) #calib_path, base_path, output_file